function [pdu, pdl,gamma]=epade( np, ns, ip, k0, c0, dr)
...
% g = (1-nu*x)**2*cdexp(alp*log(1+x)+ci*sig*(-1+sqrt(1+x)))
  if ip==1   % Split-step Pade approximation
    nu=0;
    alp=0;
  elseif ip==2   % Self-starter approximation (2-D)
    nu=1;
    alp=-0.5;
